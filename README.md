...
* Public cloud providers offer serverless platforms where you deploy  **functions** , not servers.
* You don’t manage containers or VMs directly; the platform runs your code on demand.
* Examples:  **AWS Lambda** ,  **Google Cloud Functions** ,  **Azure Functions** .
* Serverless and containers share the goal of simplifying app delivery. Under the hood, serverless platforms still run **containers** orchestrated by a control plane.
* Benefits:
  * Less infra to manage, lower ops overhead and cost
  * Pay only for execution time
  * Developers focus on code; platform handles scaling and scheduling
* Popular Kubernetes serverless frameworks:
  * **Fission** ,  **Kubeless** ,  **OpenWhisk** , **OpenFaaS**
* To use serverless on Kubernetes, you install one of these frameworks on your cluster. Ops still manages the cluster; devs deploy functions.
* Why serverless on Kubernetes:
  * Speed up delivery on a platform you already use
  * Focus on code, not infra
  * Simple, inexpensive, low-maintenance with autoscaling, resource limits, min/max scaling

### Kubeless Introduction

* **Kubeless** is a Kubernetes-native serverless framework for deploying small functions without managing the underlying infra.
* Runs on top of your K8s cluster and leverages Kubernetes for autoscaling, routing, monitoring and troubleshooting.
* Treats triggers as  **events** . Supports:
  * **HTTP** triggers (exposed as Services)
  * **Pub/Sub** triggers (Kafka, NATS)
  * **Scheduled** triggers (CronJobs)
* Open source, with a UI for deploying functions.
* Supports many languages: Python, Ruby, Node.js, PHP, Go, etc.
* CLI is broadly Lambda-like.

> Note: Kubeless is archived and not compatible with modern Kubernetes versions. The rest of this section uses  **OpenFaaS** , an actively maintained alternative that achieves the same goals.

### Install Serverless on Kubernetes with OpenFaaS

This guide sets up **OpenFaaS** on your cluster, then deploys and invokes a simple Python function.

### Prerequisites

* A running Kubernetes cluster (Minikube, kind, kubeadm, etc.)
* `kubectl` configured for that cluster
* **Helm** installed

### Install the OpenFaaS Platform

```bash
# Add the repo and create namespaces
...
```bash
# Port-forward the OpenFaaS gateway (keep this running in a separate terminal)
...
# Fetch the admin password from the secret and log in
...
```bash
# Create a new function scaffold with the Python 3 template
faas-cli new --lang python3 hello --prefix=""
```

> If you don’t have templates locally, pull one:
>
...
### Invoke and Manage the Function

```bash
# Invoke the function (echoes the request body)
...
That’s it: you’ve installed a serverless framework on Kubernetes, created a function, deployed it, and invoked it.





# Section 28 — Microservices in Kubernetes (with Istio)

A practical, current guide to designing, deploying, and operating microservices on Kubernetes using  **Istio** . This replaces the legacy Helm v2/Tiller and Istio 1.3 instructions with modern workflows tested against Istio 1.26/1.27 and Helm v3.

---

### What you’ll learn

* Microservices basics on Kubernetes: Services, Deployments, versioning
* Service mesh fundamentals: sidecars, gateways, routing, mTLS, telemetry
* Installing Istio the **2025 way** (istioctl or Helm v3)
* Exposing services with an Istio **Gateway** and **VirtualService**
* Progressive delivery: **canary** releases with weighted traffic
* **Retries** and timeouts for resiliency
* Observability: Prometheus, Grafana, Kiali, Jaeger

> Notes up front
>
> * **Don’t use Helm v2/Tiller or Istio 1.3.** Use **Helm v3** and a recent Istio (1.26/1.27 at time of writing). ([Helm](https://helm.sh/docs/intro/install/?utm_source=chatgpt.com "Installing Helm"))
> * Prefer **`istioctl`** for simplest installs. Helm v3 is also supported if you need GitOps-style control. ([Istio](https://istio.io/latest/docs/setup/install/?utm_source=chatgpt.com "Istio / Installation Guides"))
> * Istio’s networking APIs are **v1** now. Use `apiVersion: networking.istio.io/v1` for `Gateway`, `VirtualService`, etc. ([Istio](https://istio.io/latest/blog/2024/v1-apis/?utm_source=chatgpt.com "Introducing Istio v1 APIs"))

---

# Architecture overview

* **Kubernetes** runs your microservices as Pods, exposes them via Services, and handles scheduling and scaling.
* **Istio** adds:
  * **Sidecar data plane** (Envoy) per Pod for traffic control, security, and telemetry
  * **Control plane** (`istiod`) to program sidecars
  * **Gateways** to bring traffic into/out of the mesh
  * **Policies** for routing, timeouts, retries, canary, mTLS
* For ingress you can use Istio’s own API objects or  **Kubernetes Gateway API** . Istio supports both and is moving toward Gateway API as default. This guide shows Istio APIs first, then Gateway API notes. ([Istio](https://istio.io/latest/docs/tasks/traffic-management/ingress/gateway-api/?utm_source=chatgpt.com "Kubernetes Gateway API"))
...
* **Local, easiest:**
  ...
  * **kind** (Docker-based): good for fast test clusters
* **Cloud:** any managed cluster (GKE/EKS/AKS). If you build your own with  **kOps** , follow the current docs rather than old blog snippets. ([kOps](https://kops.sigs.k8s.io/cli/kops_create_cluster/?utm_source=chatgpt.com "Kops create cluster - kOps - Kubernetes Operations"))

### Tooling

* **kubectl** configured for your cluster
* **Helm v3** (no Tiller) ([Helm](https://helm.sh/docs/intro/install/?utm_source=chatgpt.com "Installing Helm"))
* **curl** and optionally **hey** or **wrk** for load testing

---

# Installing Istio (current methods)

### Option A — Install with `istioctl` (recommended)

```bash
# Download the latest Istio and add istioctl to PATH
curl -L https://istio.io/downloadIstio | sh -
cd istio-1.*    # enter the extracted folder
export PATH="$PWD/bin:$PATH"

# Install the "demo" profile (includes defaults suitable for a lab)
istioctl install --set profile=demo -y

# Enable automatic sidecar injection in your app namespace
kubectl label namespace default istio-injection=enabled
```

Docs: install with `istioctl`; getting started. ([Istio](https://istio.io/latest/docs/setup/install/istioctl/?utm_source=chatgpt.com "Install with Istioctl"))

### Option B — Install with Helm v3

```bash
# Add official Istio charts and update
helm repo add istio https://istio-release.storage.googleapis.com/charts
helm repo update

# Create istio-system and install base, control plane, and a gateway
kubectl create namespace istio-system || true
helm install istio-base istio/base -n istio-system
helm install istiod istio/istiod -n istio-system --wait

# Ingress gateway (Helm-managed)
helm install istio-ingress istio/gateway -n istio-system
```

Docs: install with Helm; ambient Helm repo is the same origin. ([Istio](https://istio.io/latest/docs/setup/install/helm/?utm_source=chatgpt.com "Install with Helm"))
...
We’ll deploy a simple Node.js HTTP service, then add a second version for canary.

### Service and Deployment v1
...
kubectl apply -f nodejs-deploy-v1.yaml
```

---

# Exposing the app with an Istio Gateway

This uses Istio’s API (`Gateway` + `VirtualService`) with **v1** API versions.
...
spec:
  selector:
    istio: ingressgateway   # matches the istio-ingressgateway service
  servers:
  - port:
      number: 80
      name: http
...
spec:
  hosts: ["*"]
  gateways: ["nodejs-gateway"]
  http:
  - route:
    - destination:
        host: nodejs.default.svc.cluster.local
        port:
          number: 8080
```
...
# Get an address to hit:
kubectl -n istio-system get svc istio-ingressgateway
# If it's a LoadBalancer, use EXTERNAL-IP:80
# For NodePort or minikube, use:
minikube service -n istio-system istio-ingressgateway --url
```

Reference for `Gateway` and `VirtualService` v1. ([Istio](https://istio.io/latest/docs/reference/config/networking/gateway/?utm_source=chatgpt.com "Gateway"))

---

# Canary deployment with weighted routing

Create a second deployment (`v2`) and shift a small percentage of traffic to it.

```yaml
# nodejs-deploy-v2.yaml
apiVersion: apps/v1
kind: Deployment
metadata:
  name: nodejs-v2
...
Define **subsets** (version-aware load balancing) and a **VirtualService** with weights:
...
```bash
kubectl apply -f nodejs-deploy-v2.yaml
kubectl apply -f canary-routing.yaml
```

Generate traffic and watch responses shift:

```bash
# Replace with your gateway URL
hey -z 30s -q 10 http://<GATEWAY_URL>/
```

You can progressively adjust weights to 20/80, 50/50, then 100/0 for full cutover. VirtualService weighting is part of Istio’s core traffic management. ([Istio](https://istio.io/latest/docs/reference/config/networking/virtual-service/?utm_source=chatgpt.com "Virtual Service"))

---

# Resiliency: retries and timeouts

Add a retry policy so transient network blips don’t fail user requests.
...
This tells Envoy to retry failed calls with a per-try timeout, shielding your app from intermittent failures. ([Istio](https://istio.io/latest/docs/reference/config/networking/virtual-service/?utm_source=chatgpt.com "Virtual Service"))

---

# Observability (Prometheus, Grafana, Kiali, Jaeger)

Istio’s “demo” profile emits metrics; you can add UI add-ons for a lab via  **samples/addons** . Then forward ports or expose via a Gateway. ([FOSS TechNix](https://www.fosstechnix.com/installing-istio-service-mesh-with-observability-add-ons-on-ubuntu-24-04-lts/?utm_source=chatgpt.com "Installing Istio Service Mesh with Observability Add-ons on Ubuntu ..."))

```bash
# Install add-ons (replace branch to match your Istio version)
...
Access locally with port-forward:

```bash
kubectl -n istio-system port-forward svc/grafana 3000:3000    # Grafana
kubectl -n istio-system port-forward svc/kiali   20001:20001  # Kiali
kubectl -n istio-system port-forward svc/jaeger  16686:16686  # Jaeger
```

* Kiali shows mesh topology and routing; integrates with Grafana and Jaeger. ([Istio](https://istio.io/latest/docs/ops/integrations/kiali/?utm_source=chatgpt.com "Kiali"))
* You can also expose dashboards through a Gateway using the pattern in “Remotely Accessing Telemetry Addons.” ([Istio](https://istio.io/latest/docs/tasks/observability/gateways/?utm_source=chatgpt.com "Remotely Accessing Telemetry Addons - Istio"))

> If you prefer hosted dashboards, Grafana Cloud has an Istio integration with prebuilt dashboards and alerts. ([Grafana Labs](https://grafana.com/docs/grafana-cloud/monitor-infrastructure/integrations/integration-reference/integration-istio/?utm_source=chatgpt.com "Istio integration | Grafana Cloud documentation"))

---

# Optional: use the Kubernetes Gateway API

Istio supports the **Gateway API** and may make it the default. If you choose this route, you’ll create `GatewayClass`, `Gateway`, and `HTTPRoute` instead of Istio’s `Gateway`/`VirtualService`. The install guides explain both options. ([Istio](https://istio.io/latest/docs/tasks/traffic-management/ingress/gateway-api/?utm_source=chatgpt.com "Kubernetes Gateway API"))

---

# Hands-on flow (quick checklist)

### 1) Cluster ready

* minikube/kind/GKE/EKS up, kubectl works
...
* Apply `nodejs-service.yaml` and `nodejs-deploy-v1.yaml`

### 4) Expose ingress

* Apply `gateway-virtualservice.yaml`, test via ingress address

### 5) Add v2 and canary

* Apply `nodejs-deploy-v2.yaml`
* Apply `canary-routing.yaml`
* Load test and tweak weights

### 6) Add resiliency

* Apply `retries-timeouts.yaml` (or fold policy into your main VirtualService)

### 7) Observability

* Apply add-ons (Prometheus/Grafana/Jaeger/Kiali)
* Port-forward or expose via a Gateway
* Inspect metrics, traces, topology

---

# Troubleshooting tips

* **Pods stuck “Pending” or CrashLoopBackOff** in `istio-system`: give the cluster more memory/CPU or check container runtime and CNI compatibility.
* **No sidecars injected** : confirm namespace label `istio-injection=enabled` and that MutatingWebhookConfiguration exists. ([Istio](https://istio.io/latest/docs/setup/install/istioctl/?utm_source=chatgpt.com "Install with Istioctl"))
* **Ingress has no external IP** on local clusters: use `minikube service -n istio-system istio-ingressgateway --url` or port-forward the gateway Service.
* **CRDs not found** errors: ensure you installed via `istioctl` or Helm **base** chart before `istiod`. ([Istio](https://istio.io/latest/docs/setup/install/helm/?utm_source=chatgpt.com "Install with Helm"))
* **Dashboards missing** : install add-ons for your exact Istio branch (`release-1.26`, `release-1.27`, etc.). ([FOSS TechNix](https://www.fosstechnix.com/installing-istio-service-mesh-with-observability-add-ons-on-ubuntu-24-04-lts/?utm_source=chatgpt.com "Installing Istio Service Mesh with Observability Add-ons on Ubuntu ..."))
...
```bash
# Remove app and routing
kubectl delete -f canary-routing.yaml || true
kubectl delete -f nodejs-deploy-v2.yaml -f nodejs-deploy-v1.yaml -f nodejs-service.yaml || true
kubectl delete -f gateway-virtualservice.yaml || true
...
# Appendix — Why old versions are obsolete

* **Helm v2/Tiller** is retired; Helm v3 is client-only. ([Helm](https://helm.sh/docs/intro/install/?utm_source=chatgpt.com "Installing Helm"))
* **Istio 1.3 and CRD pre-installs** are ancient. Use current releases and install methods (`istioctl`/Helm). ([Istio](https://istio.io/latest/docs/setup/install/?utm_source=chatgpt.com "Istio / Installation Guides"))
* Istio networking APIs are now  **v1** ; update `apiVersion` accordingly. ([Istio](https://istio.io/latest/blog/2024/v1-apis/?utm_source=chatgpt.com "Introducing Istio v1 APIs"))
* For ingress, consider the **Gateway API** path as it’s the direction of travel. ([Istio](https://istio.io/latest/docs/tasks/traffic-management/ingress/gateway-api/?utm_source=chatgpt.com "Kubernetes Gateway API"))

---

If you follow this end-to-end, you’ll have a modern, reproducible microservices lab on Kubernetes with Istio: ingress, traffic splitting, retries, and dashboards that actually work on current releases.
